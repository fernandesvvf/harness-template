...
// impl arquivo (Postgres). Cada resumo tem expiração baseada no ttl_dias da política.
import type { EpisodicSummary } from '@harness/types'
...
export class EpisodicMemoryService {
  ...
  async store(userId: string, runId: string, summary: string): Promise<void> {
    await this.setup()
    const expiresAt =
      this.ttlDays === null ? null : new Date(Date.now() + this.ttlDays * 86_400_000).toISOString()
    await getPool().query(
      `INSERT INTO episodic_summaries (run_id, user_id, summary, expires_at)
       VALUES ($1, $2, $3, $4)
       ON CONFLICT (run_id) DO UPDATE SET summary = EXCLUDED.summary, expires_at = EXCLUDED.expires_at`,
      [runId, userId, summary, expiresAt],
    )
  }

  /** Resumos vivos (não expirados), mais recentes primeiro. */
  async getRecent(userId: string, limit = 5): Promise<EpisodicSummary[]> {
    await this.setup()
    const { rows } = await getPool().query(
      `SELECT run_id, user_id, summary, created_at, expires_at
